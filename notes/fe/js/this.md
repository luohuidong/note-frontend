...
1. 在函数体中，非显式或隐式地简单调用函数时，在[[严格模式]]下，函数内的 `this` 会被绑定到 `undefined`。在非严格模式下则会被绑定到全局对象 window/global 上。
...
要理解好上面几点，需要对[[严格模式]]、[[函数显式调用]]、[[函数隐式调用]]有一定的了解。
...
4. 执行构造函数内部的代码
5. 如果构造函数 return 非原始类型的值，则返回这个值。否则返回新对象。
